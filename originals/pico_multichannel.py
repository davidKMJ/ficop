...
channel_list = (
    ...
    11,
    12,
)  # Enable Chs 1-12, Max channels (1012)
...
while ready.value == check.value:
    assert_pico_ok(picosdk.pl1000.pl1000.pl1000Ready(handle, ctypes.byref(ready)))
...
for iteration_idx in range(9):
    ...
    print(
        f"iteration {iteration_idx}: readout {read_sample_count.value} samples across {n_channels} channels"
    )

    read_samples_numpy_varsized = numpy.array(
        read_buffer[: read_sample_count.value * n_channels]
    )  # get only the valid samples out of the read_buffer
...
time_s = numpy.linspace(
    0, (len(captured_samples[:, 0]) - 1) * interval, len(captured_samples[:, 0])
)
