import numpy as np
from cua.logger.oscilloscope import Oscilloscope
...
def setup_oscilloscope():
    ...
    print("=" * 60)
    oscilloscope = Oscilloscope(auto_configure=False)
    try:
        oscilloscope.connect()
    except RuntimeError as e:
        if motor_controller:
            motor_controller.disconnect()
        raise RuntimeError("Failed to connect to oscilloscope") from e
...
def set_motor_positions(positions):
    ...
    motor_controller.set_positions(positions)
...
def manual_search(servo_idx, margin, step, is_fine_search=False, verbose=True):
    ...
    for i, pos in enumerate(range(low_position, high_position, step)):
        ...
        if (
            not is_fine_search
            and no_update_count > NO_UPDATE_COUNT_THRESHOLD
            and manual_best_value > REASONABLE_VALUE_THRESHOLD
        ):
            break
